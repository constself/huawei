...
for (let index = 1; index < sortEl.children.length; index++) {
...
// 过滤phone_resultList数据
let showResultList = [...phone_resultList]
function filterResultListAction() {
    // 过滤数据
    showResultList = phone_resultList.filter(function (item) {
        let isFlag = true
